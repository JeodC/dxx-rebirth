...
static void game_draw_marker_message(grs_canvas &canvas)
{
	if (MarkerState.DefiningMarkerMessage())
	{
		...
		gr_printf(canvas, game_font, 0x8000, (LINE_SPACING(game_font, game_font) * 5) + FSPACY(1), "Marker: %s%c", Marker_input.data(), Marker_input[Marker_input.size() - 2] ? 0 : '_');
	}
}
#endif
...
static void show_netplayerinfo(grs_canvas &canvas)
{
	...
	gr_printf(canvas, game_font, x, y, "Game mode: %s", GMNames.valid_index(gamemode) ? GMNames[gamemode] : "INVALID");
	y += line_spacing;
