...
static void multi_reset_object_texture(object_base &objp)
{
	...
	if (id)
	{
		...
		auto &model = Polygon_models[pobj_info.model_num.dsx];
...
static void MultiLevelInv_CountPlayerInventory()
{
	...
	auto &Current = MultiLevelInv.Current;
...
#if DXX_BUILD_DESCENT == 2
		Current[powerup_type_t::POW_SMISSILE1_1] += player_info.secondary_ammo[secondary_weapon_index::flash];
		...
		Current[powerup_type_t::POW_MERCURY_MISSILE_1] += player_info.secondary_ammo[secondary_weapon_index::mercury];
		Current[powerup_type_t::POW_EARTHSHAKER_MISSILE] += player_info.secondary_ammo[secondary_weapon_index::earthshaker];
#endif
                }
#if DXX_BUILD_DESCENT == 2
...
void multi_object_to_object_rw(const object &obj, object_rw *obj_rw)
{
	...
	*obj_rw = {};
	...
	switch (rtype)
	{
		...
		{
			int i;
			obj_rw->rtype.pobj_info.model_num = INTEL_INT(underlying_value(obj.rtype.pobj_info.model_num.dsx));
...
void multi_object_rw_to_object(const object_rw *const obj_rw, object &obj)
{
	...
	switch (obj.render_type)
	{
		...
		{
			int i;
			obj.rtype.pobj_info.model_num.dsx                = build_polygon_model_index_from_untrusted(INTEL_INT(obj_rw->rtype.pobj_info.model_num));
			for (i=0;i<MAX_SUBMODELS;i++)
			{
